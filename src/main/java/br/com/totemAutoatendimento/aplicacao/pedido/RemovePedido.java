...
public class RemovePedido {
    ...
    @Transactional
    public DadosDeComanda remover(DadosRemoverPedido dados) {
        Optional<Comanda> comanda = comandaRepository.buscarPeloCartao(dados.cartao(), true);
        if (comanda.isEmpty()) {
            throw new ObjetoNaoEncontradoException("Comanda aberta com cartão " + dados.cartao() + " não encontrada!");
        }
        Optional<Pedido> pedido = repository.buscarPeloId(dados.id());
        if (pedido.isEmpty()) {
